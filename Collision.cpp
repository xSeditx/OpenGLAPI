...
// Force =  Axis Displacement   * Spring Factor - Damping Factor
 ...
 {
     float DamperCoefficient = 2; // Dampening Factor
     float K = .1;
     for(CollisionSphere *List: Collider)
     {
         ...
         {                  // Returns Distance from Colliding object or FALSE for no Collision
              ...
              Vec3  AxisDisplacement = (Body.Position - List->Body.Position);
...
#if 1            
              Body.Force -=       (AxisDisplacement * (-K)) - (Body.Velocity       * DamperCoefficient); 
              List->Body.Force += (AxisDisplacement * (-K)) - (List->Body.Velocity * DamperCoefficient);
#else              
               float m1, m2, x1, x2;
               Vec3 v1temp, v1, v2, v1x, v2x, v1y, v2y, x(Position - List->Position);
               ...
               Velocity = Vec3( v1x*(m1-m2)/(m1+m2) + v2x*(2*m2)/(m1+m2) + v1y );
               List->Velocity = Vec3( v1x*(2*m1)/(m1+m2) + v2x*(m2-m1)/(m1+m2) + v2y );

#endif
         }
     float GroundHeight = Terrain::GROUND->SampleTerrain(Body.Position.x, 
                                                        Body.Position.z, this) ;
